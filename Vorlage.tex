...
\definecolor{circlecolor}{RGB}{100,100,250}

\newlength{\ratingsize}
\setlength{\ratingsize}{5pt}
\newcommand{\ratingsep}{0.25em}

\newcommand{\ratingcircle}[1]{%
  \begingroup
  \begin{tikzpicture}[baseline=-0.6ex]
    \draw[circlecolor, line width=0.6pt] (0,0) circle (\ratingsize);
    \pgfmathsetmacro{\ang}{360*(#1)}
    \ifdim\ang pt>0pt
      \fill[circlecolor]
        (0,0) -- (90:\ratingsize)
        arc[start angle=90, end angle=90+\ang, radius=\ratingsize] -- cycle;
    \fi
  \end{tikzpicture}%
  \endgroup
}
...
\newcommand{\rating}[1]{%
  \foreach \v in {#1}{%
    \ratingcircle{\v}\hspace{\ratingsep}%
  }%
}
